...
export interface ImageCropCommandArgs {
  input: string;
  x: number;
  y: number;
  width: number;
...
export interface ImageCropJobResult {
  id: string;
  status: string;
  toolName: string;
  toolVersion: string;
...
function getOutputFileId(job: ImageCropJobResult): string | null {
...
export interface ImageCropDependencies {
...
function createDefaultDependencies(): ImageCropDependencies {
  return {
    apiRequest,
    uploadCommand,
    waitJobCommand,
...
async function downloadOutputFile(
  ...
  if (!response.ok) {
    throw new Error(`Failed to download cropped file ${outputFileId}.`);
  }
...
export async function imageCropCommand(
  args: ImageCropCommandArgs,
  dependencies: Partial<ImageCropDependencies> = {},
): Promise<ImageCropJobResult> {
  const deps = {
  ...
  if (args.output) {
    ...
    if (!outputFileId) {
      throw new Error('The crop job did not produce an output file.');
    }
